...
class FloodTagsRetriever:
    ...
    def get_all_events_in_AOI(self,
                              start_date : datetime,
                              end_date : datetime,
        ...
        assert isinstance(aoi_geom, BaseGeometry), (
                'Expected input "aoi_geom" to be a shapely BaseGeometry object but a %s was input.'%type(aoi_geom))

        payload = {"sources": ",".join(sources),
                   "date.since": start_date.strftime("%Y-%m-%dT%H:%M:%SZ"),
                   "date.until": end_date.strftime("%Y-%m-%dT%H:%M:%SZ"),
                   "location.bbox": ",".join([str(coord) for coord in aoi_geom.bounds]),
                   "format":"geojson"}

        output = None
        for next_timeout in self.config.query_intervals:
            try:
                res = requests.get(self.config.event_export_end_point,
                    params=payload,
                ...
                gdf["locationNames"] = gdf["locationName"].apply(lambda x: str(x))
                gdf.set_geometry("geometry")

                #Query is via bounding box, so some points may be outside of the watershed AOI. Drop them.
        ...
        if output is None:
            ...
            warnings.warn("No data was retrieved from FloodTags.")
    ...
    def get_tag_information(self,
        ...
        def _get_payload(tag_ids: Optional[Union[pd.Series, List[str]]],
            ...
            if aoi_geom is not None:
                assert isinstance(aoi_geom, BaseGeometry)
                payload['location.bbox'] = ",".join([str(coord) for coord in aoi_geom.bounds])
        ...
        elif output['total'] == payload['view.tags.limit']:
...
def analyze_watersheds(start_date : datetime,
                       ...
                       bbox : Tuple,
                       sources : List[str],
                       flood_classes : List[str] = None) -> gpd.GeoDataFrame:
...
def _parse_text_evidence(evidence_dict : dict) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    watershed_gdf = analyze_watersheds(start_date=start_date,
                                       end_date=end_date,
                                       sources= sources,
                                       flood_classes=flood_classes,
                                       bbox= bbox)
